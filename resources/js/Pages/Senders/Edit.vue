...
const form = useForm({
    ...
    from_email: props.sender.from_email,
    from_name: props.sender.from_name,
    smtp_host: props.sender.smtp_host || '',
    smtp_port: props.sender.smtp_port || '587',
    smtp_username: props.sender.smtp_username || '',
    smtp_password: '',
    smtp_encryption: props.sender.smtp_encryption || 'tls',
    is_default: props.sender.is_default,
});

const submit = () => {
    form.put(route('senders.update', props.sender.id));
};
</script>

<template>
    <Head title="Edit Sender" />
